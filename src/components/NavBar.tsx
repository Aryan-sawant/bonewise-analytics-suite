...
const NavBar = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300 py-4 px-6 md:px-12", isScrolled ? "bg-white/80 backdrop-blur-sm shadow-sm" : "bg-transparent")}>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          {user ? <div className="flex items-center gap-4">
              ...
              <Link to="/auth?tab=signup">
                <Button size="sm" className="bg-gradient-to-r from-blue-500 to-indigo-600 border-none">Sign up</Button>
              </Link>
            </div>}
        </nav>
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && <div className="md:hidden fixed inset-0 top-[72px] bg-white/95 backdrop-blur-md z-40 animate-fade-in">
          <nav className="p-6 flex flex-col h-full">
            ...
            {user ? <div className="mt-auto pb-8">
                <div className="mb-4 p-4 bg-gray-50 rounded-xl text-sm">
                  ...
                  <p className="text-xs mt-1 text-blue-500">{user.userType === 'doctor' ? 'Doctor Account' : 'User Account'}</p>
                </div>
                <Button variant="outline" className="w-full justify-start gap-2 rounded-xl" onClick={logout}>
                  <LogOut size={16} />
                  <span>Logout</span>
                </Button>
                ...
                <Link to="/auth?tab=signup">
                  <Button className="w-full rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600">Sign up</Button>
                </Link>
              </div>}
          </nav>
        </div>}
